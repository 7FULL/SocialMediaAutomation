...
function PlatformDetail() {
  ...
  const handleAddAccount = async () => {
    try {
      ...
      // Remove category_id for non-YouTube platforms
      if (platformName !== 'YouTube') {
        delete accountData.category_id;
      }
  ...
  const handleUpdateAccount = async () => {
    try {
      ...
      await updateAccount(platformName, selectedAccount.name, updateData);
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      min="15"
                      max="180"
                      value={accountForm.clip_duration}
                      onChange={(e) => setAccountForm(prev => ({ ...prev, clip_duration: parseInt(e.target.value) || 57 }))}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                      placeholder="57"
                    />
                    <p className="text-xs text-gray-500 mt-1">
                      {platformName === 'YouTube' ? 'YouTube Shorts: 15-60 seconds' : 
                       platformName === 'TikTok' ? 'TikTok: 15-180 seconds' : 
